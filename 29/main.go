	...
	for _, article := range articles {
		...
		for i, kvStr := range sRe.Split(buf, -1) {
			...
			if k == "国旗画像" {
				...
				url := fmt.Sprintf("https://commons.wikimedia.org/w/api.php?format=json&action=query&titles=File:%s&prop=imageinfo&&iiprop=url", v)

				imageUrl, err := getUrl(url)
...
func getUrl(url string) (imageURL string, err error) {
	...
	var data map[string]Query

	if err := json.Unmarshal(bufbody.Bytes(), &data); err != nil {
		return "", err
	}

	for k, _ := range data["query"].Pages {
		for k, v := range data["query"].Pages[k].ImageInfo[0] {
			if k == "url" {
				return v, nil
			}
		}
